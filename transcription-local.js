// Work Agent - Local Whisper Voice Engine v2
...
function waPreferredMic(devices){
  ...
  return devices.find(d=>d.deviceId===WA_LOCAL_VOICE.selectedDeviceId)||devices.find(d=>/family|analog|internal|built.?in|alc256/i.test(d.label||''))||devices.find(d=>!/^predeterminado$|^default$/i.test((d.label||'').trim())&&!/redmi|bluetooth/i.test(d.label||''))||devices[0];
}
async function waLocalPopulateSelector(){
  const select=document.querySelector('#mic-select');if(!select)return;
  const devices=await waLocalInputs();select.innerHTML='';
...
async function waLocalHealth(){
  ...
  try{const r=await fetch(WA_LOCAL_VOICE.health,{signal:c.signal});return r.ok&&!!(await r.json()).ok}catch(e){return false}finally{clearTimeout(t)}
}
async function waOpenSelectedMic(){
  const devices=await waLocalInputs(),preferred=waPreferredMic(devices),ordered=[];
  if(preferred)ordered.push(preferred);devices.forEach(d=>{if(!ordered.some(x=>x.deviceId===d.deviceId))ordered.push(d)});
  let lastErr=null;
...
function waRms(){
  if(!waAnalyser)return 0;const buf=new Uint8Array(waAnalyser.fftSize);waAnalyser.getByteTimeDomainData(buf);let sum=0;for(const v of buf){const x=(v-128)/128;sum+=x*x}return Math.sqrt(sum/buf.length);
}
...
function waBeginRecording(manual=false){
  if(waLocalRecording||!waLocalStream)return;
  const mime=waLocalMime();waLocalChunks=[];waManualMode=manual;
  ...
  waLocalRecording=true;waSpeechStartedAt=Date.now();waLastVoiceAt=Date.now();
...
function waLocalStop(){
  if(!waLocalRecording)return;waLocalRecording=false;
  if(waLocalTimer){clearTimeout(waLocalTimer);waLocalTimer=null}
  if(waManualMode)setVoiceState('ready','Transcribiendo local…');
  try{if(waLocalRecorder?.state!=='inactive')waLocalRecorder.stop()}catch(e){}
}
function waMonitorLoop(){
  if(!waAnalyser)return;
  const now=Date.now(),level=waRms(),speaking=level>=WA_LOCAL_VOICE.levelThreshold;
  if(speaking){waLastVoiceAt=now;if(!waLocalRecording&&waHandsFree){waBeginRecording(false);setVoiceState('listening',`Manos libres · escuchando…`)}}
...
async function waEnsureStream(){
  ...
  try{waLocalStream=await waOpenSelectedMic();waCreateAnalyser(waLocalStream);waMonitorLoop();return true}catch(err){feedback(`No pude abrir el micrófono: ${err.name||err.message}`,true,'No pude abrir el micrófono.');return false}
}

async function waTranscribeBlob(blob,mime){
  const c=new AbortController(),t=setTimeout(()=>c.abort(),20000);
  try{const r=await fetch(WA_LOCAL_VOICE.endpoint,{method:'POST',headers:{'Content-Type':mime},body:blob,signal:c.signal});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||`HTTP ${r.status}`);return String(d.text||'').trim()}finally{clearTimeout(t)}
}
function waStripWake(text){
  const escaped=WA_LOCAL_VOICE.wakeWord.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  const re=new RegExp(`(^|\\b)${escaped}\\b[,:;.!?\\s-]*`,'i');
  if(!re.test(text))return null;return text.replace(re,'').trim();
}
function waSayShort(text){try{if(typeof speak==='function')speak(text)}catch(e){}}
async function waHandleHandsFreeText(text){
  const command=waStripWake(text),armed=Date.now()<waHandsFreeArmedUntil;
  if(command===null&&!armed){setVoiceState('ready',`Manos libres · esperando “${WA_LOCAL_VOICE.wakeWord}”`);return}
  const clean=(command===null?text:command).trim();
  if(!clean){waHandsFreeArmedUntil=Date.now()+8000;listeningBeep();setVoiceState('listening',`Sí, Rodrigo · decime`);return}
  waHandsFreeArmedUntil=0;commandInput.value=clean;setVoiceState('ready',`Jarvis: “${clean}”`);runCommand(clean,true);
}
async function waLocalFinish(manual){
  const mime=waLocalRecorder?.mimeType||'audio/webm',blob=new Blob(waLocalChunks,{type:mime});
  try{
    const text=await waTranscribeBlob(blob,mime);if(!text)throw new Error('Whisper no devolvió texto.');
    if(manual){commandInput.value=text;setVoiceState('ready',`Whisper: “${text}”`);runCommand(text,true)}else await waHandleHandsFreeText(text);
  }catch(err){if(manual)feedback(`Whisper local: ${err.message}`,true,'No pude transcribir eso.');setVoiceState('ready',waHandsFree?`Manos libres · esperando “${WA_LOCAL_VOICE.wakeWord}”`:'Whisper local listo')}
  if(manual&&!waHandsFree)waCloseStream();
}

async function waLocalStart(){
  if(waLocalRecording){waLocalStop();return}
  waLocalDisableLegacy();if('speechSynthesis'in window)window.speechSynthesis.cancel();
  setVoiceState('ready','Conectando Whisper local…');
  if(!await waLocalHealth()){setVoiceState('ready','Whisper local desconectado');feedback('Whisper local no está corriendo.',true,'El motor local de voz no está iniciado.');return}
  if(!await waEnsureStream())return;
  commandInput.value='';commandFeedback.textContent='';setVoiceState('listening','Whisper local · hablá ahora');waBeginRecording(true);
}
async function waToggleHandsFree(){
  const btn=document.querySelector('#handsfree-toggle');
  if(waHandsFree){waHandsFree=false;waHandsFreeArmedUntil=0;if(waLocalRecording)waLocalStop();waCloseStream();if(btn){btn.textContent='Manos libres: OFF';btn.classList.remove('active')}setVoiceState('ready','Whisper local listo');return}
  ...
  if(!await waEnsureStream())return;
  waHandsFree=true;if(btn){btn.textContent='Manos libres: ON';btn.classList.add('active')}setVoiceState('ready',`Manos libres · decí “${WA_LOCAL_VOICE.wakeWord}”`);waSayShort(`Manos libres activo. Decí ${WA_LOCAL_VOICE.wakeWord} cuando me necesites.`)
}
...
window.WA_VOICE_PRO={enabled:()=>true,start:waLocalStart,stop:waLocalStop,listInputs:waLocalInputs,version:'local-whisper-2.0'};

// Work Agent Voice Brain v1.5
// Parser por entidades + contexto. Si Voz PRO esta activa, Chrome no toma el microfono.
...
function waVoiceProOwnsMic(){
  try{
    return !!(window.WA_VOICE_PRO && window.WA_VOICE_PRO.enabled && window.WA_VOICE_PRO.enabled());
  }catch(e){
    return false;
  }
}
...
function waMergeWithContext(entities){
  ...
  return {sex:entities.sex||waVoiceContext.sex,age:entities.age!==null?entities.age:waVoiceContext.age,unit:entities.unit||waVoiceContext.unit||'anios'};
}
...
function waResolveFollowUp(raw,entities){
  ...
  const merged={sex:entities.sex||prev.sex,age:entities.age!==null?entities.age:prev.age,unit:entities.unit||prev.unit};
...
function waCommandKey(raw){
  const q=normalizeVoiceCommand(raw),entities=waExtractEntities(raw),follow=waResolveFollowUp(raw,entities),merged=follow||waMergeWithContext(entities);
...
extractMiniHubQuery=function(raw){
  const entities=waExtractEntities(raw),follow=waResolveFollowUp(raw,entities),merged=follow||waMergeWithContext(entities);
...
function waStartFreshFollowUp(attempt=0){
  // Voz PRO es duena exclusiva del microfono. Chrome queda como fallback manual.
  if(waVoiceProOwnsMic()) return;
  if(typeof recognition==='undefined'||!recognition||!waConversationFresh()) return;
  if(isListening){if(attempt<8)setTimeout(()=>waStartFreshFollowUp(attempt+1),100);return;}
  finalTranscript='';interimTranscript='';manualStop=false;beeped=false;
  waConversation.awaiting=true;waConversation.followUpUntil=Date.now()+WA_FOLLOWUP_TTL;waArmFollowUpTimeout();
  try{recognition.start();setVoiceState('listening','Seguimos · esperá el pip');}catch(e){if(attempt<8)setTimeout(()=>waStartFreshFollowUp(attempt+1),140);}
}

function waSpeakAndThenListen(text){
  if(!('speechSynthesis' in window)){if(!waVoiceProOwnsMic())setTimeout(()=>waStartFreshFollowUp(),400);return;}
  window.speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(text);
  u.lang=preferredVoice?.lang||'es-AR';if(preferredVoice)u.voice=preferredVoice;u.rate=.92;u.pitch=1;u.volume=1;
  u.onend=()=>{if(!waVoiceProOwnsMic())setTimeout(()=>waStartFreshFollowUp(),400);};
  window.speechSynthesis.speak(u);
}

const waOriginalRunCommand=runCommand;
runCommand=function(value=null,fromVoice=false){
  const raw=String(value!==null?value:commandInput.value).trim(),q=normalizeVoiceCommand(raw);
  ...
  const entities=waExtractEntities(raw),follow=waResolveFollowUp(raw,entities),merged=follow||waMergeWithContext(entities);
  if(merged.sex&&merged.age!==null){const mini=extractMiniHubQuery(raw);if(mini){waResetContext();waRememberMini(mini);answerMiniHub(mini);return;}}
  const hasMiniEntity=!!entities.sex||entities.age!==null||!!entities.unit;
  if(fromVoice&&hasMiniEntity){
    if(!merged.sex){commandFeedback.textContent='Tengo la edad. Decime mujer o varón.';commandFeedback.classList.remove('error');waVoiceContext.updatedAt=Date.now();if(!waVoiceProOwnsMic())setTimeout(()=>waStartFreshFollowUp(),220);return;}
    if(merged.age===null){commandFeedback.textContent='Tengo el sexo. Decime la edad.';commandFeedback.classList.remove('error');waVoiceContext.updatedAt=Date.now();if(!waVoiceProOwnsMic())setTimeout(()=>waStartFreshFollowUp(),220);return;}
  }
  waStopFollowUp();waResetContext();return waOriginalRunCommand(value,fromVoice);
};
...
answerMiniHub=function(mini){
  ...
  const [altura,peso]=row[mini.sex],sexo=mini.sex==='m'?'Mujer':'Varón',edadTxt=mini.unit==='meses'?`${mini.age} ${mini.age===1?'mes':'meses'}`:`${mini.age} ${mini.age===1?'año':'años'}`;
  commandFeedback.textContent=`${sexo} · ${edadTxt} → ${altura} cm · ${peso} kg`;commandFeedback.classList.remove('error');waRememberMini(mini);waSpeakAndThenListen(`${altura} centímetros. ${peso} kilos.`);
};

if(typeof recognition!=='undefined'&&recognition){
  recognition.onresult=e=>{
    let newFinal='',newInterim='';
    for(let i=e.resultIndex;i<e.results.length;i++){const text=e.results[i][0].transcript.trim();if(e.results[i].isFinal)newFinal+=` ${text}`;else newInterim+=` ${text}`;}
    if(newFinal)finalTranscript=`${finalTranscript} ${newFinal}`.replace(/\s+/g,' ').trim();interimTranscript=newInterim.trim();
    const heard=fullTranscript();if(heard){commandInput.value=heard;setVoiceState('listening',`Escuchando: “${heard}”`);const complete=isCompleteVoiceCommand(heard),delay=waConversation.awaiting?(complete?220:700):(complete?60:520);scheduleFinish(delay);}
  };
  recognition.onspeechend=()=>{const heard=fullTranscript(),complete=isCompleteVoiceCommand(heard),delay=waConversation.awaiting?(complete?180:620):(complete?50:450);scheduleFinish(delay);};
}
